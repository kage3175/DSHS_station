...
def ScoreWrite(songnums, highest):
    outfilescore = open('./doc/score.txt', "w")
    for i in range(songnums):
...
def main(idx, songnums) : # 드디어! 100줄만에! main 함수 입성!
    ...
    track3note=[0 for i in range(1000)]
    zz=[0 for i in range(3)]
    linesinf3 = [0 for i in range(10000)]
    string1=''
    temp2=0
    ...
    if idx == 0 :
        for i in range(n*3) :
            if i%3 == 0 :
                linesinf3[i]=int(linesinf2[i]) - 10
            else :
                linesinf3[i]=int(linesinf2[i])
    else :
        for i in range(n*3) :
            linesinf3[i]=int(linesinf2[i])

    for i in range(n):  # 총 노트의 수만큼 for문을 돌려서 각 노트의 x 좌표값을 알맞게 바꿔준다.
        x[i] = 10 + 60 * (linesinf3[i*3+1])
    ...
    for i in range(n):  # imgg 리스트에 노트의 이미지를 불러와준다.
        aaa=ran.randint(1,1500) # 랜덤 자연수 생성임을 모르는 무지한 이가 있겠는가
        if aaa > 3 : # 매우 큰 확률이라네. 대충 997/1500 쯤? 맞나? 랜덤 생성 메커니즘은 몰라서...
            imgg[i] = pygame.image.load('./image/note/t1.png')
            z[i] = 0 # 이미지는 윗줄에서 바꿨지만 그걸 컴퓨터가 인식할 수는 없으니까... 이짓 추가로 해줘서 판별하게 해야함. 나중가면 알게 될 거 ㅇㅇ
        elif aaa > 0 :
            imgg[i] = pygame.image.load('./image/note/t2.png')
            z[i] = 3
        else : # 위에꺼의 여집합이니까... 알지? 그냥 확률 작은 거야. 난 너희에게 포인트 많이 주기 싫거든
    ...
    while running : # running은 뭐다? TRUE다!<< 어허 이 사람 큰 일 낼 사람일세... True 지 TRUE 가 아니라우. TRUE 쓰면 오류남... ㄹㅇ? 사실 나도 모름 ㅎㅎ;; 걍 1 쓰는 것보다 깐지나 보여서 running 쓰는 거임... 딱히 의미 없고
        ...
        if cnt >= linesinf3[3*n-3]+300 :
            Endi=1
        if Pausei == 0 :
            for i in range(n):
                if y[i] <= 420 and cnt >= linesinf3[i*3]: # 아무 일도 일어나지 않으면
                    y[i] += 3 # y좌표 3씩 추가
                if y[i] > 420 and y[i] != 500:  # 노트의 y좌표가 420을 넘어가면 노트를 게임 밖으로 내보내고, miss 처리한다.
                    CountMissTime[linesinf3[i*3+1]] = 10 # miss 이펙트 처리를 위해 해당 변수에 10 대입
                    y[i] = 500  # miss, great, perfect 처리된 노트는 y좌표 500으로 이동시킨다.
                    miss += 1
                    combo=0
                    if linesinf3[i*3+1]==0 : # 사실 여기부터는 나도 무슨 의미인지 잘 모르는 코드가 많음... 떠오른 거 그냥 써놓고는 주석 안 달아놔서... 이 얘기를 왜 지금하냐고? 이 줄이 무슨 의미인지 나도 잘 모르니까! 깔깔깔깔깔깔
                        m[0]+=1
                    elif linesinf3[i*3+1]==1:
                        m[1]+=1
                    else :
                        m[2]+=1
            ...
            if event.type == KEYDOWN: # 키를 눌렀을 때
                if Pausei == 0 :
                    if event.key == K_RIGHT: # 방향키 오른쪽
                        for i in range(n) : # 모든 노트를 일일이 다 검사함... 사실 귀찮아서 그러는 거긴 한데 프로그램 돌아가는 속도 저하의 가장 큰 원인이 이 줄에 있당
                            if linesinf3[3*i+1] == 2 and y[i] >= 280 and y[i] <= 420:# 오른쪽 키를 눌렀을 때 세 번째 레인에서, y좌표가 280과 420 사이일 때 해당 순번의 노트를 처리하는 이벤트이다.
                                if m[2]==linesinf3[3*i+2] : # 해당 순번의 노트에 대해서만 액션을 취하도록 해주는 행
                                    ShowScore(ScoreChange(y[i], n)) # 위에 가서 어떤 함수인지 보고 오셔용. 참 쉽죠?
                                    ShowJudgement(2) # 얘도
                                    if z[i] : # 특수 노트인 경웅에
                    ...
                    if event.key == K_LEFT: # 난 절대로 네버 에버 두.번.말.하.지.않.는.다.
                        for i in range(n):
                            if linesinf3[i*3+1] == 0 and y[i] >= 280 and y[i] <= 420:
                                if m[0]==linesinf3[3*i+2] :
                                    ShowScore(ScoreChange(y[i], n))
                                    ShowJudgement(0)
                                    if z[i] :
                    ...
                    if event.key == K_DOWN: # 난 두번만 말하지 않는 게 아니지. 세 번 말하지도 않지.
                        for i in range(n):
                            if linesinf3[3*i+1] == 1 and y[i] >= 280 and y[i] <= 420:
                                if m[1]==linesinf3[3*i+2] :
                                    ShowScore(ScoreChange(y[i], n))
                                    ShowJudgement(1)
                                    if z[i] :
                                        if JudgeNote==3 or JudgeNote==2 :
        ...
        for i in range(n) : # 노트 위치를 상시 출력해준다. 오버
            if y[i] != 500 and y[i] != 10 :
        ...
        if Endi==1 :
            ...
            screen.blit(imgend, (50,150))
            aaa=round(score,2)
            textSurfaceObjEndScore = fontObj4.render('Score : '+str(aaa)+'%', True, WHITE)
            textRectObjEndScore = textSurfaceObjEndScore.get_rect();
            textRectObjEndScore.center = (162, 180)
            screen.blit(textSurfaceObjEndScore, textRectObjEndScore)
            if score > highest[idx]:  # 이번 기록이 종전 최고기록보다 높을 경우 최고기록을 변경해준다.
                textSurfaceObjEndBest = fontObj4.render('Best Score : ' + str(aaa) + '%', True, WHITE)
                textRectObjEndBest = textSurfaceObjEndBest.get_rect();
                textRectObjEndBest.center = (162, 210)
            else :
                textSurfaceObjEndBest = fontObj4.render('Best Score : '+str(highest[idx])+'%', True, WHITE)
                textRectObjEndBest = textSurfaceObjEndBest.get_rect();
                textRectObjEndBest.center = (162, 210)
            textSurfaceObjEndJudgement = fontObj2.render("PERFECT : " + str(perfect) + " GREAT : " + str(great) + " MISS : " + str(miss), True, WHITE)
            textRectObjEndJudgement = textSurfaceObjEndJudgement.get_rect();
            textRectObjEndJudgement.center = (162, 235)
            screen.blit(textSurfaceObjEndBest, textRectObjEndBest)
            screen.blit(textSurfaceObjEndJudgement, textRectObjEndJudgement)
            if event.type == MOUSEBUTTONUP and event.button == LEFT:  # 마우스 왼쪽 클릭하면
                position = pygame.mouse.get_pos()  # 위치 받아와서
                if position[0] in range(86,237) :
                    if position[1] in range(257,284) :
                        Quiti = 1
